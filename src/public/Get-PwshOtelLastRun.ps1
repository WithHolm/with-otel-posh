function Get-WotelLastRun {
    ...
    Set-WotelTraceOptions -IgnoreLogs Enabled

    $LastTrace = $global:wotel.values | ? { $_.options.IgnoreLogs -eq 'Disabled' -and $_.global -ne $true } | sort-object -Property createdTime -Descending | select-object -first 1
    ...
    if ($Raw) {
        return ($LastTrace.spans.values.events | Sort-Object Timestamp)
    }
    #return logs
    if ($Logs) {
        #return all logs that are above the severity level. the write to std out accepts a otel log object as input parameters
        ($LastTrace.spans.values.events | Sort-Object Timestamp) | Where-Object { $_.SeverityNumber -ge ([int]$Severity) } | ForEach-Object {
            $out = $_
            Invoke-WotelWriterConsole @out -IgnoreDebugLevel
            # Write-BoltLogToStdOut @out -IgnoreDebugLevel
        }
    }
    ...
    if ($Statistics) {
        ...
        $LongestName = ($spans.values.Name | % { $_.length } | measure -Maximum).Maximum
        ...
        for ($i = $spans.Count - 1; $i -gt -1; $i--) {
            ...
            if ($startlog -and $endlog) {
                ...
                $ChildDurations = ($span.children | % { $_.ActiveDurationMs } | Measure-Object -Sum).Sum
